...
const EXTRACTION_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      location: { type: 'string', description: 'City/region from the resume header or address' },
      linkedin_url: { type: 'string', description: 'LinkedIn profile URL if present, else null' },
      github_url: { type: 'string', description: 'GitHub profile URL if present, else null' },
      portfolio_url: { type: 'string', description: 'Personal website or portfolio URL if present, else null' },
      years_of_experience: {
        type: 'number',
        description: 'Total years of professional experience inferred from work history. Use earliest start year vs current year.',
      },
      seniority_level: {
        type: 'string',
        enum: ['entry', 'mid', 'senior', 'staff', 'lead', 'principal', 'director', 'vp', 'c_suite'],
        description: 'Seniority inferred from most recent job title and years of experience',
      },
      target_roles: {
        type: 'array',
        items: { type: 'string' },
        description: 'Up to 3 role titles that best represent this candidate based on their experience',
      },
      target_industries: {
        type: 'array',
        items: {
        ...
        description: 'Up to 3 industries inferred from the companies and domains the candidate has worked in',
      },
    },
    required: [],
  },
...
Be precise: only extract information that is clearly present.
For inferred fields like seniority and industry, use your best judgment based on the content.
Return null for any field not determinable from the text.`
...
export async function extractProfileFromResume(
  ...
  const trimmed = resumeText.slice(0, 6000) // haiku context — enough for most resumes
  ...
  if (extracted.years_of_experience != null) profile.years_of_experience = Math.min(30, Math.max(0, Math.round(extracted.years_of_experience)))
